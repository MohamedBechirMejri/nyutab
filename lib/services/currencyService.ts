...
const exchangeRatesCache: Record<
	string,
	{ data: CurrencyData; timestamp: number }
> = {};
// Cache TTL in milliseconds (15 minutes instead of 1 hour)
const CACHE_TTL = 15 * 60 * 1000;

// Cache for historical rates to reduce API calls
const historicalRatesCache: Record<
	string,
	{ data: CurrencyHistory[]; timestamp: number }
> = {};
...
export const fetchExchangeRates = async (
	...
	try {
		...
		try {
			console.log(
				`Fetching exchange rates for ${baseCurrency} from fallback API`,
			);
...
const fetchHistoricalRatesAlternative = async (
	...
	console.log(
		`==== USING FALLBACK HISTORICAL API for ${baseCurrency}/${targetCurrency} ====`,
	);
		...
		try {
			...
			if (response.ok) {
				...
				if (data.rates && targetCurrency in data.rates) {
					...
					console.warn(
						`Fallback API: Target currency ${targetCurrency} not found in response`,
					);
				}
			} else {
			...
			console.error(
				`Fallback API: Error fetching data for date ${
					date.toISOString().split("T")[0]
				}:`,
				error,
			);
...
export const fetchHistoricalRates = async (
	...
	if (cachedData && Date.now() - cachedData.timestamp < HISTORICAL_CACHE_TTL) {
		console.log(
			`Using cached historical rates data for ${baseCurrency}/${targetCurrency}`,
		);
	...
	console.log(
		`Fetching historical rates for ${baseCurrency}/${targetCurrency}, no cache hit`,
	);
			...
			if (response.ok) {
				...
				if (targetCurrency.toLowerCase() in ratesData) {
					...
					console.log(
						...
						Object.keys(ratesData).join(", "),
					);
				}
			} else {
				console.warn(
					`API returned error status ${response.status} for ${formattedDate}`,
				);
				try {
	...
	if (result.length === 0) {
		console.warn(
			`Primary API failed for ${baseCurrency}/${targetCurrency}. Trying fallback API...`,
		);
		try {
			...
			if (fallbackResult.length > 0) {
				console.log(
					`Fallback API returned ${fallbackResult.length} data points`,
				);
	...
	console.log(
		`==== FETCH HISTORICAL END: ${baseCurrency}/${targetCurrency} ====`,
	);
...
export const useCurrencyData = (baseCurrency: CurrencyCode = "USD") => {
	...
	useEffect(() => {
		...
		if (timeUntilNextUpdate > 0 && timeUntilNextUpdate < 30 * 60 * 1000) {
			console.log(
				`Setting up refresh timer for ${timeUntilNextUpdate}ms from now`,
			);
			...
			console.log(
				"Next update time is in the past, scheduling refresh in 15 minutes",
			);
...
export const convertCurrency = (
	...
	throw new Error(
		`Cannot convert from ${fromCurrency} to ${toCurrency} with available rates`,
	);
};
