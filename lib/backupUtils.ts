...
export const SETTINGS_KEYS = ['favorites', 'memes', 'position', 'feed'] as const
...
export const parseBackup = async (file: File): Promise<CompleteBackup> => {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = event => {
      try {
        const backup = JSON.parse(event.target?.result as string) as CompleteBackup
 ...
 */
export const restoreSelectedData = (backup: CompleteBackup, keys: BackupKey[]) => {
 ...
 */
export const restoreSelectedSettings = (backup: CompleteBackup, settingsKeys: SettingsKey[]) => {
