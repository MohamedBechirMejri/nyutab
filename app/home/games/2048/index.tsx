...
const X2048 = () => {
    ...
    const handleMove = (newBoard: any[]) => {
      return newBoard.map(c => {
        if (
          c.y === null ||
          c.y === 0 ||
          newBoard.filter(cell => cell.x === c.x && cell.y === c.y - 1).length > 1
        )
          return c
    ...
    const handleMove = (newBoard: any[]) => {
      return newBoard.map(c => {
        if (
          c.x === null ||
          c.x === 0 ||
          newBoard.filter(cell => cell.x === c.x - 1 && cell.y === c.y).length > 1
        )
          return c
  ...
  const mergeTiles = () => {
    setBoard(board => {
      ...
      occupiedCells.forEach(c => {
        occupiedPositions[c.x + ' ' + c.y] = [...(occupiedPositions[c.x + ' ' + c.y] || []), c]
      })
  ...
  const addNewTile = (board: any[]) => {
    ...
    } while (occupiedPositions.includes(randomPosition.x + ' ' + randomPosition.y))
