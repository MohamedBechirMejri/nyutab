...
export default function FitGirl() {
  ...
  const genres = useMemo(() => {
    const genres = [...new Set(posts.flatMap(([_, post]) => post.info.genres || []))]
    ...
    <div className="flex flex-col h-full bg-black/25 backdrop-blur-3xl w-full rounded-2xl overflow-hidden p-8 pt-4 relative select-none">
      ...
      <div className="pb-6 pt-0 flex justify-between items-end gap-4">
        <Select
          ...
          selected={sortBy}
          setSelected={sortBy => setSortBy((sortBy as 'latest' | 'largest' | 'A-Z') || 'latest')}
          label="Sort by"
          noCreate
        />
        ...
        {posts.length ? (
          sortedPosts.length ? (
            ...
            <div className="flex items-center justify-center h-full">
              <span className="text-2xl font-bold text-center">No Results..</span>
            </div>
          )
        ) : (
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex w-full justify-between pr-4 items-start">
            ...
            <button
              ...
              onClick={() => window.open(`https://fitgirl-repacks.site/${id}`, '_blank')}
            >
              See Post
            </button>
          </div>
